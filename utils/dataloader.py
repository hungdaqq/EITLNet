...
class SegmentationDataset_train(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image_path_jpg = os.path.join(
            os.path.join(self.dataset_path, "Images"), name + ".jpg"
        )
        image_path_png = os.path.join(
            os.path.join(self.dataset_path, "Images"), name + ".png"
        )
        if os.path.isfile(image_path_jpg):
            ...
            img = Image.open(
                os.path.join(os.path.join(self.dataset_path, "Images"), name + ".png")
            )
        else:
            img = Image.open(
                os.path.join(os.path.join(self.dataset_path, "Images"), name + ".tif")
            )

        gt_path_png = os.path.join(
            os.path.join(self.dataset_path, "Masks01"), name + ".png"
        )
        if os.path.isfile(gt_path_png):
            mask = Image.open(
                os.path.join(os.path.join(self.dataset_path, "Masks01"), name + ".png")
            )
        else:
            mask = Image.open(
                os.path.join(os.path.join(self.dataset_path, "Masks01"), name + ".tif")
            )

        # Data augmentation
        img, mask = self.get_random_data(
            img, mask, self.input_shape, random_flag=self.train
        )
    ...
    def get_random_data(
        ...
        image_data = cv2.resize(
            decoded_image, (image_data.shape[1], image_data.shape[0])
        )
