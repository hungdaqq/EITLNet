...
def get_net(num_classes=2, phi="b2", pretrained=True, dual=True):
    model = SegFormer(
        num_classes=num_classes, phi=phi, pretrained=pretrained, dual=dual
    )
    return model
...
def parse_args():
    ...
    parser.add_argument(
        "--model_path",
        type=str,
        default="weights/weights_EITL_new.pth",
        help="Path to model weights",
    )
    parser.add_argument(
        "--total_epoch", type=int, default=100, help="Total number of epochs"
    )
    parser.add_argument(
        "--batch_size", type=int, default=8, help="Batch size for training"
    )
    parser.add_argument(
        "--init_lr", type=float, default=0.0005, help="Initial learning rate"
    )
    parser.add_argument(
        "--save_period",
        type=int,
        default=5,
        help="Frequency of saving model (in epochs)",
    )
    parser.add_argument(
        "--save_dir",
        type=str,
        default="./log/b2_network/",
        help="Directory to save logs and models",
    )
    parser.add_argument(
        "--data_path", type=str, default="./dataset/", help="Path to the dataset"
    )
...
if __name__ == "__main__":
    ...
    with open(os.path.join(data_path, "Split/train.txt"), "r") as f:
        train_lines = f.readlines()
    with open(os.path.join(data_path, "Split/test.txt"), "r") as f:
